...
public class TUIRect : TUINeedUpdateBase
{
	...
	public bool PtInControl(Vector2 m_position)
	{
		float num = size.x / 2f;
		float num2 = size.y / 2f;
		if (m_position.x >= 0f - num + base.transform.position.x && m_position.x <= num + base.transform.position.x && m_position.y >= 0f - num2 + base.transform.position.y && m_position.y <= num2 + base.transform.position.y)
		{
			return true;
		}
		return false;
